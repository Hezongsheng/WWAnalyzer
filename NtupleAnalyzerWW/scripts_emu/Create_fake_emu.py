...
for j in range(0,ncat):
    ...
    for k in range(0,nbhist):
        ...
        h0.Add(fTop.Get("em_"+str(j)+"_anti/top"+postfix),-1)#Get the data-MCbkg
        for i in range(0,h0.GetSize()-2):
            if h0.GetBinContent(i)<0:
                h0.SetBinError(i,max(0,h0.GetBinError(i)+h0.GetBinError(i)))#hard to understand
        ...
        h0R.Add(fTop.Get("emR_"+str(j)+"_anti/top"+postfix),-1)#Get the data-MCbkg
